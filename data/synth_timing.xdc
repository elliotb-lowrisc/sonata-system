...
set rst_2sync_pins [get_pins -filter {REF_PIN_NAME =~ rst_ni} -of $rst_2sync_cells]
# Filter out any that do not have a real timing path (fail to find leaf cell).
set rst_2sync_endpoints [filter [all_fanout -endpoints_only -flat $rst_2sync_pins] IS_LEAF]
set_max_delay $clk_sys_ns -to $rst_2sync_endpoints
